...
;(function(window) {
	...
	function openItem(item) {
		...
		classie2.add(zoomer, 'zoomer--active');
		// disallow scroll
		...
		onEndTransition(zoomer, function() {
			...
			// no scrolling
			classie2.add(bodyEl, 'noscroll');
			classie2.add(contentEl, 'content--open');
			var contentItem = document.getElementById(item.getAttribute('data-content'))
			classie2.add(contentItem, 'content__item--current');
			classie2.add(contentItem, 'content__item--reset');


			// reset zoomer transform - back to its original position/transform without a transition
			classie2.add(zoomer, 'zoomer--notrans');
	...
	function closeContent() {
		...
		classie2.remove(contentEl, 'content--open');
		classie2.remove(contentItem, 'content__item--current');
		classie2.remove(bodyEl, 'noscroll');
		...
		onEndTransition(contentItem, function(ev) {
			classie2.remove(this, 'content__item--reset');
			...
			setTimeout(function() {	
				classie2.remove(zoomer, 'zoomer--notrans');
				classie2.remove(zoomer, 'zoomer--active');
				zoomer.style.WebkitTransform = 'translate3d(0,0,0) scale3d(1,1,1)';
				zoomer.style.transform = 'translate3d(0,0,0) scale3d(1,1,1)';
			}, 25);
	...
	function navigate(dir) {
		...
		dynamics.animate(nextEl, { opacity: 1, translateX: 0 }, {
			...
			friction: 600,
			complete: function() {
				items.forEach(function(item) { classie2.remove(item, 'slide--current'); });
				classie2.add(itemNext, 'slide--current');
			}
		});
